...
@Component({
	selector: 'app-localcurrencytransfer',
	templateUrl: './LocalCurrencyTransfer.component.html',
	styleUrls: ['./LocalCurrencyTransfer.component.scss'],
	providers: [
		DatePipe,
...
export class LocalCurrencyTransferComponent implements OnInit {
	...
	fillForm(_CurrencyTransfer: CurrencyTransfer) {
		...
		this.myForm.patchValue({
			...
			DeliveryLocationID: _CurrencyTransfer.DeliveryLocationID,
			DeliveryLocation: this.initObject(_CurrencyTransfer.DeliveryLocation),
	...
	DeliveryLocation_AutoComplete(name?: string) {
		this._CityService.City_Auto(name).subscribe(res => {
			this.DeliveryLocationList = res.Data;
		});
	}

	onDeliveryLocationChanged($event?: any) {
		if ($event) {
			const selected: any = $event.option.value;
			this.get('DeliveryLocationID').setValue(selected.ID);
		} else {
			this.get('DeliveryLocation').setValue(null);
			this.get('DeliveryLocationID').setValue(null);
		}
	}
