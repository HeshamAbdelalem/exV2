...
@Component({
	selector: 'app-sendcards',
	templateUrl: './SendCards.component.html',
	styleUrls: ['./SendCards.component.scss'],
...
export class SendCardsComponent implements OnInit {
	...
	Display() {
		...
		const _SendCard: any = this.myForm.value;
		_SendCard.PageIndex = 1;
		_SendCard.PageSize = 1000;
		return this._SendCardService.GetSendCardPage(_SendCard).subscribe(res => {
	...
	Delete(id: number) {
		Swal.fire({
			...
			if (result.value) {
				this._SendCardService.DeleteSendCard(id).subscribe(res => {
					if (!res.Success) {
